...
async function callGeminiApi(promptText: string): Promise<string | null> {
    ...
    try {
        ...
        if (candidate?.finishReason && candidate.finishReason !== "STOP") {
            ...
            if (candidate.finishReason === "SAFETY") { /* handle safety */ return null; }
        }
        const responseText = candidate?.content?.parts?.[0]?.text;
        if (responseText) {
            console.log(`[GeminiService] Received response (${responseText.trim().length} chars).`);
            return responseText.trim();
        } else { /* handle missing text */ return null; }
...
export async function enhanceCodeWithGemini(
    ...
    if (expectsCode) {
        ...
        } else { /* handle extraction failure */ return { type: 'error', content: `Failed to extract code block.` }; }
    } else if (expectsText) {
